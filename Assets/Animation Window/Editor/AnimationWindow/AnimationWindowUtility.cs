...
using UnityEditor;
using UnityEditor.PlayEm;
using UnityEditor.Animations;
using UnityEngine;
using Object = UnityEngine.Object;

using TangentMode = UnityEditor.AnimationUtility.TangentMode;
using CurveRenderer = UnityEditor.PlayEm.CurveRenderer;
using IntCurveRenderer = UnityEditor.PlayEm.IntCurveRenderer;
using BoolCurveRenderer = UnityEditor.PlayEm.BoolCurveRenderer;
using NormalCurveRenderer = UnityEditor.PlayEm.NormalCurveRenderer;
using CurveWrapper = UnityEditor.PlayEm.CurveWrapper;
using CurveUtility = UnityEditor.PlayEm.CurveUtility;
using CurveSelection = UnityEditor.PlayEm.CurveSelection;
using CurveEditor = UnityEditor.PlayEm.CurveEditor;
...
namespace UnityEditorInternal.PlayEm {
	static internal class AnimationWindowUtility {
		...
		public static void CreateDefaultCurves(AnimationWindowState state, AnimationClip animationClip, EditorCurveBinding[] properties) {
			properties = UnityEditor.PlayEm.RotationCurveInterpolation.ConvertRotationPropertiesToDefaultInterpolation(animationClip, properties);
			...
			if (curve.isPPtrCurve) {
				...
				if (type == typeof(bool)) {
					...
					if (animationCurve.length <= 1) {
						...
						if (keyIndex != -1) {
							UnityEditor.PlayEm.CurveUtility.SetKeyModeFromContext(animationCurve, keyIndex);
		...
		public static EditorCurveBinding[] PropertyModificationsToEditorCurveBindings(PropertyModification[] modifications, GameObject rootGameObject, AnimationClip animationClip) {
			...
			for (int i = 0; i < modifications.Length; ++i) {
				...
				if (AnimationUtility.PropertyModificationToEditorCurveBinding(modifications[i], rootGameObject, out binding) != null) {
					EditorCurveBinding[] additionalBindings = UnityEditor.PlayEm.RotationCurveInterpolation.RemapAnimationBindingForAddKey(binding, animationClip);
		...
		public static void SyncTimeArea(UnityEditor.PlayEm.TimeArea from, UnityEditor.PlayEm.TimeArea to) {
		...
		public static void DrawOutOfRangeOverlay(Rect rect, Color color, float startOfClipPixel, float endOfClipPixel) {
			...
			if (startOfClipPixel > rect.xMin) {
				...
				UnityEditor.PlayEm.TimeArea.DrawVerticalLine(startRect.xMax, startRect.yMin, startRect.yMax, lineColor);
			}
			...
			UnityEditor.PlayEm.TimeArea.DrawVerticalLine(endRect.xMin, endRect.yMin, endRect.yMax, lineColor);
		}
		...
		public static void ControllerChanged() {
			foreach (PlayEmAnimationWindow animationWindow in PlayEmAnimationWindow.GetAllAnimationWindows())
